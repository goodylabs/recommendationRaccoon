...
import { updateRecommendationsFor, updateSimilarityFor, updateWilsonScore } from './algorithms';
...
const changeRating = async function (
  ...
  config.logger.debug(`changeRating: ${userId} ${itemId}, liked: ${options.liked}`);
  ...
  config.logger.debug(`changeRating: ${userId} ${itemId}, result2: ${result2}`);

  if (updateRecommendations && result2 > 0) {
    config.logger.debug(`changeRating: ${userId} ${itemId}, updateRecommendations`);
